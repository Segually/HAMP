...
## IMPORTANT: IDA must be running
**If the IDA MCP server is not connected, STOP and tell the user immediately.** Do not attempt protocol reverse engineering without it — guessing wire formats wastes time and introduces bugs. Remind the user to start IDA with the MCP plugin before continuing.
